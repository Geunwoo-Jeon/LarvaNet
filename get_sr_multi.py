...
from utils import image_utils_chop
          ...
          for i in range(2):
            if i == 0:
              ...
              for j in range(4):
                ...
                out_img = image_utils_chop.upscale_with_chop_forward(model=model, input_image=rot_flip_img, scale=args.scale, overlap_size=args.chop_overlap_size)
                tmp = np.rot90(out_img, 4-j, axes=(1,2))
                tmp_img += tmp[:,:,::-1]
            else:
              for k in range(4):
                rot_img = np.rot90(input_image, k, axes=(1,2))
                out_img = image_utils_chop.upscale_with_chop_forward(model=model, input_image=rot_img, scale=args.scale, overlap_size=args.chop_overlap_size)
                tmp_img += np.rot90(out_img, 4-k, axes=(1,2))
          output_image = tmp_img / 8
      else:
        with torch.no_grad():
          output_image = image_utils_chop.upscale_with_chop_forward(model=model, input_image=input_image, scale=args.scale, overlap_size=args.chop_overlap_size)
