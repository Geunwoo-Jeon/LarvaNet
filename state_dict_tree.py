...
def main():
    ...
    # parameter mixing
    new_dict = model.model.branch_0.state_dict()
    for key in new_dict:
        new_dict[key] = model.model.branch_0.state_dict()[key] + model.model.branch_1.state_dict()[key]
        new_dict[key] = new_dict[key]/2
    model.model.branch_0.load_state_dict(new_dict)
    ...
    for scale in scale_list:
        ...
        with torch.no_grad():
            for image_index in range(num_images):
                ...
                print(f'x4, {image_index + 1}/{num_images}, psnr={psnr:.2f}')

            branch_psnr = np.mean(psnr_list)
            print(f'avg psnr = {branch_psnr:.4f}')
