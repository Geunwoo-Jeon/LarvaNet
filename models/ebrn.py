...
class BRM(nn.Module):
  def __init__(self, num_channels, scale, back_project=True):
    ...
    self.up_block = nn.ConvTranspose2d(in_channels=num_channels, out_channels=num_channels, kernel_size=kernel_size, stride=stride, padding=padding)
    ...
    self.down_block = nn.Conv2d(in_channels=num_channels, out_channels=num_channels, kernel_size=kernel_size, stride=stride, padding=padding)
...
class EBRNModule(nn.Module):
  def __init__(self, args, scale):
    ...
    self.feature_extract = nn.Sequential(
      nn.Conv2d(in_channels=3, out_channels=num_filters*4, kernel_size=kernel_size, stride=stride, padding=padding),
      nn.PReLU(num_filters*4),
      nn.Conv2d(in_channels=num_filters*4, out_channels=num_filters, kernel_size=kernel_size, stride=stride, padding=padding),
      nn.PReLU(num_filters),
      nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=kernel_size, stride=stride, padding=padding),
      nn.PReLU(num_filters),
    )

    self.fusion_layers = nn.ModuleList()
    for _ in range(self.num_brms - 1):
      self.fusion_layers.append(nn.Conv2d(in_channels=num_filters, out_channels=num_filters, kernel_size=kernel_size, stride=stride, padding=padding))
    ...
    self.recon_layer = nn.Conv2d(in_channels=num_filters*self.num_brms, out_channels=num_colors, kernel_size=kernel_size, stride=stride, padding=padding)
  ...
  def forward(self, x):
    ...
    for i in range(self.num_brms - 1):
      sr = self.fusion_layers[i](sr + sr_list[-(i+1)])
      out_list.append(sr)

    x = self.recon_layer(torch.cat(out_list, dim=1))
