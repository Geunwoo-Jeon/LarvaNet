...
class MAMBlock(nn.Module):
    def __init__(self, num_channels, weight=1.0, distill_rate=0.25):
        ...
        self.attention = MAMLayer(num_channels=num_channels)
        self.weight = weight
...
class MAMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        dev_map = x - mean
        dev_map = torch.pow(dev_map, 2)

        mean_dev = dev_map.mean(dim=1, keepdim=True).repeat(1, C, 1, 1)
        std_dev = dev_map.std(dim=1, keepdim=True).repeat(1, C, 1, 1)

        dev_map = (dev_map - mean_dev) / std_dev

        self.modulation_map_CSI = dev_map
