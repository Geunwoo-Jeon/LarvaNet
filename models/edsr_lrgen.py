...
class EDSRModule(nn.Module):
  def __init__(self, args, scale):
    ...
    self.down_conv = nn.Conv2d(in_channels=3, out_channels=num_conv_features, kernel_size=1+(scale*2), stride=scale, padding=scale)
    ...
    self.last_conv = nn.Conv2d(in_channels=num_conv_features, out_channels=3, kernel_size=1, stride=1, padding=0)
