...
class MDSR(BaseModel):
    ...
    def upscale(self, input_tensor, scale):
        # get SR
        input_tensor = input_tensor.to(self.device)
        output_tensor = self.model.forward_upscale(input_tensor, scale=scale)

        # finalize
        return output_tensor.detach().cpu().numpy()

    def feature(self, input_list, scale):
        # numpy to torch
        input_tensor = torch.as_tensor(input_list, dtype=torch.float32, device=self.device)
        # get SR
        output_tensor = self.model.forward_feature(input_tensor, scale=scale)
...
class MDSRModule(nn.Module):
    ...
    def forward_feature(self, x, scale):
        x = self.sub_mean(x)
        x = self.head(x)
    ...
    def forward_upscale(self, x, scale):
