...
class DiffRatio(nn.Module):
  def __init__(self):
    super(DiffRatio, self).__init__()
  ...
  def forward(self, x):
    ...
    diff_rg = torch.unsqueeze(diff_rg, 1)
    diff_gb = torch.unsqueeze(diff_gb, 1)
    diff_rb = torch.unsqueeze(diff_rb, 1)

    # print(diff_rg.shape)

    self.ratio_rg_rb = torch.div(diff_rg, torch.abs(diff_rb)+1)
    self.ratio_rb_rg = torch.div(diff_rb, torch.abs(diff_rg)+1)

    self.ratio_gb_rb = torch.div(diff_gb, torch.abs(diff_rb)+1)
    self.ratio_rb_gb = torch.div(diff_rb, torch.abs(diff_gb)+1)

    self.ratio_rg_gb = torch.div(diff_rg, torch.abs(diff_gb)+1)
    self.ratio_gb_rg = torch.div(diff_gb, torch.abs(diff_rg)+1)

    output = torch.cat((self.ratio_rg_rb, self.ratio_rb_rg, self.ratio_gb_rb, self.ratio_rb_gb, self.ratio_rg_gb, self.ratio_gb_rg), dim=1)
    return output
...
class EDSRModule(nn.Module):
  def __init__(self, args, scale):
    ...
    self.diff_ratio = DiffRatio()

    self.mean_shift = MeanShift([114.4, 111.5, 103.0], sign=1.0)
    self.first_conv = nn.Conv2d(in_channels=9, out_channels=args.edsr_conv_features, kernel_size=3, stride=1, padding=1)
  ...
  def forward(self, x):
    x_ratio = self.diff_ratio(x)
    x_rgb = self.mean_shift(x)
    # print(x_ratio.shape)
    x = torch.cat((x_rgb, x_ratio), dim=1)
