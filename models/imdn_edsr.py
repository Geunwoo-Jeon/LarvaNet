...
class IMDN_AIM2019(BaseModel):
  ...
  def prepare(self, is_training, scales, global_step=0):
    ...
    # PyTorch model
    self.model = IMDN_EDSR_Module(args=self.args, scale=self.scale)
    if (is_training):
...
class IMDBlock(nn.Module):
  def __init__(self, num_channels, distill_rate=0.25):
    ...
    self.resblock2 = ResidualBlock(num_channels=num_channels-1*self.distilled_channels)
    self.resblock3 = ResidualBlock(num_channels=num_channels-2*self.distilled_channels)
    self.resblock4 = ResidualBlock(num_channels=num_channels-3*self.distilled_channels)

    self.summary_conv = nn.Conv2d(in_channels=self.distilled_channels*4, out_channels=num_channels, kernel_size=1, stride=1, padding=0)
  
  def forward(self, x):
    res1 = self.resblock1(x)
    distilled_res1, remaining_res1 = torch.split(res1, (self.distilled_channels, self.num_channels-1*self.distilled_channels), dim=1)
    res2 = self.resblock2(remaining_res1)
    distilled_res2, remaining_res2 = torch.split(res2, (self.distilled_channels, self.num_channels-2*self.distilled_channels), dim=1)
    res3 = self.resblock3(remaining_res2)
    distilled_res3, remaining_res3 = torch.split(res3, (self.distilled_channels, self.num_channels-3*self.distilled_channels), dim=1)
    res4 = self.resblock4(remaining_res3)

    res = torch.cat([distilled_res1, distilled_res2, distilled_res3, res4], dim=1)
    res = self.summary_conv(res)
...
class IMDN_EDSR_Module(nn.Module):
  def __init__(self, args, scale):
    super(IMDN_EDSR_Module, self).__init__()
