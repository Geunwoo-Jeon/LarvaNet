...
class SquidNet(BaseModel):
    ...
    def prepare(self, is_training, scales, global_step=0):
        ...
        for i in range(self.args.num_modules):
            if i == 0:
                tmp_module = SquidHeadModule(args=self.args)
                self.modules.append(tmp_module)
            else:
                tmp_module = SquidLegModule(args=self.args)
                self.modules.append(nn.Sequential(self.modules[i-1], tmp_module))

        legs = self.args.num_modules - 1
        self.model = self.modules[legs]
    ...
    def train_step_squid(self, args, val_dataloader, input_list, scale, truth_list, summary=None):
        self.global_step += 1
        ...
        # train each legs
        for i in range(self.args.num_modules):
            if self.global_step < i * self.epoch:
                continue
            ...
            if self.global_step % args.step_per_epoch == 0:
                self.validate_for_train(args, val_dataloader, self.schedulers[i])
    ...
    def validate_for_train(self, args, dataloader, scheduler):
        # scheduling lr by validation
        if self.global_step % (args.step_per_epoch * 10) == 0:
            self.epoch += 1
...
class SquidLegModule(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.res_blocks(self.lrelu(fea))
        out = self.upsample(out)
        out += x
        return out
