...
class EDSR_ADL(BaseModel):
  ...
  def parse_args(self, args):
    ...
    parser.add_argument('--conv_features', type=int, default=64, help='The number of convolutional features.')
    ...
    parser.add_argument('--drop_rate', type=float, default=0.5, help='The drop rate.')
    parser.add_argument("--drop_inverse", action='store_true')
  ...
  def prepare(self, is_training, scales, global_step=0):
    ...
    # PyTorch model
    self.model = EDSR_ADL_Module(args=self.args, scale=self.scale)
    if (is_training):
...
class ResBlock(nn.Module):
  def __init__(self, num_channels, drop_rate=0.5, drop_inverse=False, weight=1.0):
...
class ADL(nn.Module):
  ...
  def forward(self, x):
    ...
    mean_tv = tmp_tv.view(N, -1).mean(dim=-1, keepdim=True).repeat(1, C)
    std_tv = tmp_tv.view(N, -1).std(dim=-1, keepdim=True).repeat(1, C)

    tmp_tv = (tmp_tv - mean_tv) / std_tv 

    modulation_map_CSI = tmp_tv.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, W, H)

    y = self.scaling(modulation_map_CSI)
...
class EDSR_ADL_Module(nn.Module):
  def __init__(self, args, scale):
    super(EDSR_ADL_Module, self).__init__()

    self.first_conv = nn.Conv2d(in_channels=3, out_channels=args.conv_features, kernel_size=3, stride=1, padding=1)
    
    res_block_layers = []
    for i in range(args.num_blocks):
      res_block_layers.append(ResBlock(num_channels=args.conv_features, drop_rate=args.drop_rate, drop_inverse=args.drop_inverse, weight=args.res_weight))
    self.res_blocks = nn.Sequential(*res_block_layers)
    self.after_res_conv = nn.Conv2d(in_channels=args.conv_features, out_channels=args.conv_features, kernel_size=3, stride=1, padding=1)

    self.upsample = UpsampleBlock(num_channels=args.conv_features, scale=scale)
    self.final_conv = nn.Conv2d(in_channels=args.conv_features, out_channels=3, kernel_size=3, stride=1, padding=1)
