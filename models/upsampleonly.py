...
class UpsampleOnly(BaseModel):
  ...
  def parse_args(self, args):
    ...
    parser.add_argument('--model_learning_rate_decay', type=float, default=0.5, help='Learning rate decay factor.')
    parser.add_argument('--model_learning_rate_decay_steps', type=int, default=200000, help='The number of training steps to perform learning rate decay.')
  ...
  def _get_learning_rate(self):
    return self.args.model_learning_rate * (self.args.model_learning_rate_decay ** (self.global_step // self.args.model_learning_rate_decay_steps))
...
class EDSRModule(nn.Module):
  def __init__(self, args, scale):
    ...
    self.upsample = UpsampleBlock(num_channels=3, scale=scale)
