...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        res1 = self.conv(x)
        res2 = self.conv(res1)
        distilled_res1, remaining_res1 = torch.split(res2, (self.distilled_channels, self.remaining_channels), dim=1)
...
class MAMLayer(nn.Module):
    def __init__(self, num_channels, kind=True):
...
class MDSRModule(nn.Module):
    def __init__(self, args, scale_list):
        ...
        self.pre_process = nn.ModuleList([
            nn.Sequential(
                ResidualBlock(num_channels=args.edsr_conv_features, weight=args.edsr_res_weight),
                ResidualBlock(num_channels=args.edsr_conv_features, weight=args.edsr_res_weight)
            ) for _ in scale_list
        ])
