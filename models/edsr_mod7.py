...
def initialize_weights(net_l, scale):
...
class ResidualBlock(nn.Module):
  def __init__(self, num_channels):
    ...
    initialize_weights(self.body, 1/33)
...
class EDSRModule(nn.Module):
  def __init__(self, args, scale):
    ...
    initialize_weights(self.first_conv, 1/33)
  
  def forward(self, x):
    x = self.mean_shift(x)
    tmp = self.lrelu(self.first_conv(x))
    nearest_feat = x.repeat_interleave(self.scale * self.scale, dim=1)
    x = torch.add(nearest_feat, tmp)
