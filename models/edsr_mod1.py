...
class EDSRModule(nn.Module):
  def __init__(self, args, scale):
    ...
    for i in range(args.edsr_res_blocks):
      res_block_layers.append(ResidualBlock(num_channels=num_conv_features, weight=args.edsr_res_weight))
    self.res_blocks = nn.Sequential(*res_block_layers)
    self.after_res_conv = nn.Conv2d(in_channels=num_conv_features, out_channels=num_conv_features, kernel_size=3, stride=1, padding=1)
