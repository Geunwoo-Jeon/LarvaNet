...
def main():
  ...
  parser.add_argument('--stage_freq', type=int, default=15000, help='The frequency of saving the trained model.')
  ...
  while (model.global_step < args.max_steps):
    ...
    if (local_train_step % args.stage_freq == 0): 
      stage = 1 - stage

    loss = model.train_step(input_list=input_list, scale=scale, truth_list=truth_list, summary=summary, stage=stage)
