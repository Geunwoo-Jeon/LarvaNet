...
class DIV2KLoader(BaseLoader):
  ...
  def prepare(self, scales):
    ...
    input_path = os.path.join('c:/aim2020/data/DIV2K_valid_HR')
  ...
  def _get_input_image(self, scale, image_name):
    ...
    if (image is None):
      image_path = os.path.join('c:/aim2020/data/DIV2K_valid_LR_bicubic', ('X%d' % (scale)), ('%sx%d.png' % (image_name, scale)))
  ...
  def _get_truth_image(self, image_name):
    ...
    if (image is None):
      image_path = os.path.join('c:/aim2020/data/DIV2K_valid_HR', ('%s.png' % (image_name)))
      image = self._load_image(image_path)
