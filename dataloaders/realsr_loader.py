...
class RealSRLoader(BaseLoader):
  ...
  def prepare(self, scales):
    ...
    input_path = os.path.join(self.args.data_truth_path, ('x%d' % (scales[0])), 'TrainHR')
    self.image_name_list = [os.path.splitext(f)[0].split('_')[0] for f in os.listdir(input_path) if f.lower().endswith('.png')]
  ...
  def _get_input_image(self, scale, image_name):
    ...
    if (image is None):
      ...
      image_path = os.path.join(self.args.data_input_path, ('x%d' % (scale)), 'TrainLR', ('%s_LR.png' % (image_name)))
  ...
  def _get_truth_image(self, scale, image_name):
    ...
    if (image is None):
      ...
      image_path = os.path.join(self.args.data_truth_path, ('x%d' % (scale)), 'TrainHR', ('%s_HR.png' % (image_name)))
      image = self._load_image(image_path)
