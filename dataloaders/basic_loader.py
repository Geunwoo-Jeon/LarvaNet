...
# Basic loader that supports single-scale images
...
class BasicLoader(BaseLoader):
  ...
  def parse_args(self, args):
    ...
    parser.add_argument('--data_input_path', type=str, default='input', help='Base path of the input images.')
    parser.add_argument('--data_truth_path', type=str, default='truth', help='Base path of the ground-truth images.')
  ...
  def _get_input_image(self, scale, image_name):
    ...
    if (self.args.data_cached):
      if (image_name in self.cached_input_image_list):
        image = self.cached_input_image_list[image_name]
        has_cached = True
    
    if (image is None):
      image_path = os.path.join(self.args.data_input_path, ('%s.png' % (image_name)))
      image = self._load_image(image_path)
    
    if (self.args.data_cached and (not has_cached)):
      self.cached_input_image_list[image_name] = image
